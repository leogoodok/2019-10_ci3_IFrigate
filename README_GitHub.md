...
1. Отправка сообщений администратору (формы) осуществляется без перезагрузки страницы с использованием JS AJAX методом POST. Перед отправкой формы выполняется проверка успешной валидации (средствами JS) обязательных полей. Обработчик отправки в файле:
```
~\docs\js\main.js
```

2. Контроль и редакторование вводимых пользователем данных. Осуществляется с использованием JS. Обработчики в файле:
```
~\docs\js\main.js
```
